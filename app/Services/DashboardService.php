...
class DashboardService
{
    ...
    public function build(User $user, int $chartDays = 30): array
    {
        ...
        $activePlan = MemorisationPracticePlan::query()
            ...
            ->latest('id')
            ->first();
        $openRecommendation = SessionRecommendation::query()
            ->where('user_id', $user->id)
    ...
    private function buildProgress(
        ...
        $surahCompletion = ($surahAyahCount > 0)
            ? (int) round(($surahMemorised / $surahAyahCount) * 100)
            : null;
    ...
    private function buildWeaknesses(User $user): array
    {
        ...
        return [
            'items' => $sorted->take(3)->values()->all(),
            'total' => $total,
            'has_more' => $total > 3,
    ...
    private function calculateStreak(User $user): int
    {
        ...
        if ($dates->first() !== $cursor->toDateString()
            && $dates->first() !== $cursor->copy()->subDay()->toDateString()) {
            return 0;
        }
