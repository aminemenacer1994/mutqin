...
assert.match(source, /skipCountdownToPlay\(\)/)
assert.match(source, /primeSessionAudioForCountdown\(\)/)
assert.match(source, /pauseAfterUnlock/)
assert.match(source, /Stay muted until finalizeCountdownPlayback/)
assert.match(source, /showCountdownOverlay && !options\.allowDuringCountdown/)
assert.match(source, /queueAmdLiveWordPatch/)
