...

{
  ...
  const completionModal = vue.match(/post-session-simple__dialog--actions-3[\s\S]*?<\/footer>/)?.[0] || ''

  assert.match(vue, /Teleport to="body"/)
  assert.match(vue, /post-session-simple/)
  assert.match(vue, /post-session-simple__actions--cards|post-session-simple__action-card/)
  ...
  assert.ok(completionModal.length > 0, 'streamlined 3-action completion modal present')
  ...
  assert.doesNotMatch(completionModal, /ps-plan__adjust|openPostSessionAdjustPlan/, 'adjust plan removed from completion modal')
  assert.doesNotMatch(completionModal, /postSessionRecommendationStep === 'confirm'/, 'no confirmation step in streamlined completion')
  ...
  assert.match(css, /\.post-session-simple__action-card/)
