...
store.set('mutqin.defaultFontSize', '160')
store.set('mutqin.defaultFontSize.42', '210')
store.set('mutqin.layoutFontSizes.42', JSON.stringify({ mushaf: 210, stacked: 140 }))
...
assert.equal(store.has('mutqin.defaultFontSize'), false)
assert.equal(store.get('mutqin.defaultFontSize.42'), '210')
assert.equal(JSON.parse(store.get('mutqin.layoutFontSizes.42')).mushaf, 210)
