...
/**
 * Whether a session range overlaps the learner's main memorisation journey place.
 *
 * @param {number} surahNumber
 * @param {number} rangeStart
 * @param {number} rangeEnd
 * @param {{ surah_number?: number, ayah_start?: number, ayah_end?: number }|null|undefined} journeyContinue
 */
export function sessionRangeOverlapsJourney(surahNumber, rangeStart, rangeEnd, journeyContinue) {
  const journeySurah = Number(journeyContinue?.surah_number || 0)
  const journeyStart = Number(journeyContinue?.ayah_start || 0)
  const journeyEnd = Number(journeyContinue?.ayah_end || journeyStart)
  const sessionSurah = Number(surahNumber || 0)
  const sessionStart = Number(rangeStart || 0)
  const sessionEnd = Number(rangeEnd || sessionStart)
  if (!journeySurah || !journeyStart || !sessionSurah || !sessionStart) return false
  if (sessionSurah !== journeySurah) return false
  return sessionStart <= journeyEnd && sessionEnd >= journeyStart
}

/**
 * @param {{ surah_name?: string, ayah_start?: number, ayah_end?: number }|null|undefined} journeyContinue
 */
export function formatJourneyContinueRangeLabel(journeyContinue) {
  const surahName = String(journeyContinue?.surah_name || '').trim()
  const start = Number(journeyContinue?.ayah_start || 0)
  const end = Number(journeyContinue?.ayah_end || start)
  if (!surahName) return ''
  if (start > 0 && end > 0 && start !== end) return `${surahName} · ${start}–${end}`
  if (start > 0) return `${surahName} · ${start}`
  return surahName
}

/** @param {string|number|Date|null|undefined} lastActivityAt */
export function daysSinceActivity(lastActivityAt, nowMs = Date.now()) {
